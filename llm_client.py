...
def llm_query(data, client, type, task, **config):
    ...
    raise RuntimeError(
        "[DEMO MODE] llm_query disabled: OpenAI is not allowed"
    )

    if type == "llamacpp":
        raise RuntimeError("llamacpp should NOT call OpenAI API")
...
def paraphrase(sentence, client, type, **kwargs):
    ...
    # If running local Llama.cpp, don't paraphrase (no API)
    if kwargs.get("language_model", None) == "llamacpp":
        return [text]   # or return original list
