...
from llama_cpp import Llama   # Locat model change from transformers + torch.generate to llama.cpp (llama-cpp-python)
...
class Evaluator(object):
    def __init__(self, args) -> None:
        ...
        # template family selection --> add llamacpp
        if "alpaca" in args.language_model:
            model = "alpaca"
        elif args.language_model == "llamacpp":      # Add llamacpp
            model = "alpaca"                         # Llamacpp uses alpaca's prompt template
            # self.client = None
        ...
        elif args.language_model == "llamacpp":
        ...
        self.public_out_path = args.output or "outputs/tmp_run"
    ...
    def create_dataset(
        ...
        # --- Llamacpp / no-tokenizer path: return prompt strings directly ---
        if tokenizer is None:
            if verbose and len(data_with_prompt) > 0:
                self.logger.info(
                     "### dataset example (no tokenizer): " + data_with_prompt[0] + "\n"
                )
    ...
    def get_generations(self, prompt_pre,  eval_src, ref_texts):
        ...
        is_llamacpp = ("llamacpp" in args.language_model)
        ...
        dataset = self.create_dataset(
            ...
            tokenizer = None if is_llamacpp else self.tokenizer,
            # model=self.model_name,
            model = "davinci" if is_llamacpp else self.model_name,
            batch_size=batch_size,
        ...
        # Append every prompt
        for prompt in tqdm(dataset):      # dataset must be list[str]
            out = self.llama(prompt, max_tokens=args.max_new_tokens, temperature=0)
            hypos.append(out["choices"][0]["text"].strip())
        return hypos
            ...
            if args.task == "cls":
                ...
                if "davinci" in args.llm_type:
                    ...
                    for data in tqdm(dataset):
                        ...
                        # print(pred)
                        hypos.append(pred)
        # LLama.cpp Path (Add New)
        elif args.language_model == "llamacpp":
            # Let create_dataset return prompt string list
            dataset_prompts = self.create_dataset(
                args.dev_file,
                eval_src,
                ref_texts,
                tokenizer = None,
                model = "davinci",       # force using the path of return strings
                batch_size = batch_size,
                prompt_num = args.prompt_num,
                prompt_pre = prompt_pre,
                task = args.task,
            ...
            for prompt in tqdm(dataset_prompts):
                out = self.llama(
                   prompt,
...
class CLSEvaluator(Evaluator):
    def __init__(self, args):
        ...
        # Llamacpp has no transformers tokenizer, cann't cal verbalizer_ids
        if ("gpt" not in args.language_model) and ("llamacpp" not in args.language_model):
            self.verbalizer_ids = [
    ...
    def forward(
        ...
        if "llamacpp" in args.language_model:
            pred_mode = "gen"
            pred = self.get_generations(prompt_pre, eval_src, ref_texts)
            pred = [remove_punctuation(i) for i in pred]
            hypos = list(map(lambda x: first_appear_pred(x, dataset_classes_list[self.args.dataset], self.logger), pred))
